...
// toggleNavbar opens or closes a responsive navbar by adding or removing CSS class nkn-open to the UL element with class nkn-navbar.
// toggle is the element which was clicked to trigger this function, ie. any element with data-nkn-role = navbar-toggle.
// When opening a responsive navbar, we add click handlers to each LI element to close the navbar when clicked.
    ...
    if (myMenu) {
        if (myMenu.classList.contains('nkn-open')) {
            ...
            for (var indx = 0; indx < myLinks.length; indx++) {
                if (myLinks[indx].dataset.nknRole != "navbar-toggle") {
